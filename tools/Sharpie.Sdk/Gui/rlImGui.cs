...
public static class rlImGui
{
    ...
    public static void Setup(bool darkTheme = true, bool enableDocking = false)
    {
        ...
        style.Colors[(int)ImGuiCol.WindowBg] = new Vector4(0.93f, 0.93f, 0.93f, 1f);

        style.FrameRounding = 3f;
        style.FrameBorderSize = 1f;
        style.Colors[(int)ImGuiCol.Button] = new Vector4(0.78f, 0.78f, 0.78f, 1f);
        style.Colors[(int)ImGuiCol.ButtonHovered] = new Vector4(0.88f, 0.88f, 0.88f, 1f);
        style.Colors[(int)ImGuiCol.ButtonActive] = new Vector4(0.68f, 0.68f, 0.68f, 1f);

        style.Colors[(int)ImGuiCol.Text] = new Vector4(0.1f, 0.1f, 0.1f, 1f);

        style.Colors[(int)ImGuiCol.Border] = new Vector4(0.5f, 0.5f, 0.5f, 1f);

        style.Colors[(int)ImGuiCol.FrameBg] = new Vector4(0.95f, 0.95f, 0.95f, 1f);
        style.Colors[(int)ImGuiCol.FrameBgHovered] = new Vector4(0.88f, 0.88f, 0.88f, 1f);
        style.Colors[(int)ImGuiCol.FrameBg] = new Vector4(0.8f, 0.8f, 0.8f, 1f);
        ...
        var embeddedFont = Helpers.GetEmbeddedFontFile();
        if (embeddedFont != null)
            unsafe
            {
                fixed (byte* pFont = embeddedFont)
                {
                    ImGui
                        .GetIO()
                        .Fonts.AddFontFromMemoryTTF((IntPtr)pFont, embeddedFont.Length, 16);
                }
            }
        else
            ImGui
                ...
                .Fonts.AddFontFromFileTTF(
                    Path.Combine(Directory.GetCurrentDirectory(), "Gui/res/Tahoma.ttf"),
                    16
                );
    ...
    public static void EndInitImGui()
    {
        ...
        unsafe
        {
            ...
            platformIO.Platform_SetClipboardTextFn = Marshal.GetFunctionPointerForDelegate(
                SetClipCallback
            );

            GetClipCallback = new GetClipTextCallback(rImGuiGetClipText);
            platformIO.Platform_GetClipboardTextFn = Marshal.GetFunctionPointerForDelegate(
                GetClipCallback
            );
        }
    ...
    private static void SetMouseEvent(
        ImGuiIOPtr io,
        MouseButton rayMouse,
        ImGuiMouseButton imGuiMouse
    )
    {
    ...
    private static void NewFrame(float dt = -1)
    {
        ...
        if (
            ...
            || Raylib.IsWindowState(ConfigFlags.HighDpiWindow)
        )
