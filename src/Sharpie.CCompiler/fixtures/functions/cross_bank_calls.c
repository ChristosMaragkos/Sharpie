...
BANK(3) int do_stuff();
BANK(3) int do_stuff() { return 42; }

// Define the banked functions in the same file (tests single-file multi-bank)
__attribute__((annotate("bank_1"))) int fetch_enemy_sprite(int enemy_id) {
    return enemy_id * 3;
}
...
int main(void) {
    ...
    // Should retrieve the pointer from the stack into r13, load '2' into r14,
    // and call SYS_FAR_CALL
    int distance = path_ptr(10, 20);

    return sprite + distance + do_stuff();
}
