...
syn keyword sharpieOpcode NOP nop MOV mov LDM ldm LDP ldp LDV ldv STV stv LDI ldi STM stm STP stp STA sta LDS lds STS sts GETSP getsp SETSP setsp ADD add SUB sub MUL mul DIV div MOD mod AND and OR or XOR xor SHL shl SHR shr CMP cmp INC inc DEC dec NOT not NEG neg PRNT prnt SIGEX sigex IADD iadd ISUB isub IMUL imul IDIV idiv IMOD imod IAND iand IOR ior IXOR ixor ICMP icmp DINC dinc DDEC ddec JMP jmp JEQ jeq JNE jne JGT jgt JLT jlt JGE jge JLE jle JC jc JNC jnc CALL call RET ret PUSH push POP pop OUT_R out_r OUT_B out_b OUT_W out_w RND rnd FLIPR flipr CAM cam GETOAM getoam SETOAM setoam GETSEQ getseq SETSEQ setseq SONG song SETCRS setcrs CRSPOS crspos DRAW draw INSTR instr OAMPOS oampos SAVE save OAMTAG oamtag CLS cls VBLNK vblnk PLAY play STOP stop INPUT input TEXT text ATTR attr SWC swc BANK bank BLITMODE blitmode MUTE mute COL col ALT alt HALT halt
